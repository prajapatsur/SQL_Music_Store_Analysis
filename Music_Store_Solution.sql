...
select billing_country, count(billing_country) as number from invoice group by billing_country order by number desc limit 1
...
select billing_city, sum(total) as Total_Sales from invoice group by billing_city order by Total_Sales desc limit 1
...
select a.artist_id,a.name, count(t.track_id) as Songs from artist a join album al on al.artist_id=a.artist_id join track t on t.album_id=al.album_id join genre g on g.genre_id=t.genre_id where g.name='Rock' group by a.artist_id order by songs desc limit 10
